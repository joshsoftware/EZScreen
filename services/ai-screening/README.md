...
- **Resume & JD Matching**: Evaluating candidate fit against job criteria to calculate `resume_score` (0–100) and `job_fit_analysis` JSONB.
...
- `POST /internal/v1/match/resume-jd` — Score candidate resume against JD (`resume_score` & `job_fit_analysis`).
