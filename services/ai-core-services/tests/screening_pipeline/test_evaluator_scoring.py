...
@pytest.mark.asyncio
async def test_evaluator_combines_llm_keyword_score_with_llm_quality():
    evaluator = _evaluator_returning(
        '{"intent": "ANSWERING", "response": "", '
        '"keyword_match_score": 5, "keywords_found": ["Docker"], '
        '"keywords_missing": ["container"], "answer_quality_score": 8, "decision": "NEXT_QUESTION", '
...
@pytest.mark.asyncio
async def test_keyword_match_score_is_taken_from_llm_and_bounded():
    evaluator = _evaluator_returning(
        '{"intent": "ANSWERING", "response": "", '
        '"keyword_match_score": 15, "keywords_found": ["docker", "Kubernetes", "container"], '
    ...
    # The LLM's score is authoritative but clamped to 0-10. "Kubernetes" was never
    # expected so it is dropped; the expected list's own spelling is preserved.
    assert result["keyword_match_score"] == 10.0
...
@pytest.mark.asyncio
async def test_missing_keyword_score_falls_back_to_llm_keyword_list():
    evaluator = _evaluator_returning(
        '{"intent": "ANSWERING", "response": "", "keywords_found": ["Docker"], '
...
@pytest.mark.asyncio
async def test_no_keyword_score_or_list_scores_zero_keyword_coverage():
    evaluator = _evaluator_returning(
        '{"intent": "ANSWERING", "response": "", '
        '"answer_quality_score": 6, "decision": "NEXT_QUESTION", "feedback": ""}'
    )
...
async def test_no_expected_keywords_uses_answer_quality_for_both_halves():
    evaluator = _evaluator_returning(
        '{"intent": "ANSWERING", "response": "", "keyword_match_score": 0, '
        '"keywords_found": [], "answer_quality_score": 7, '
        '"decision": "ASK_FOLLOW_UP", "feedback": ""}'
    )
...
@pytest.mark.parametrize(
    ("keyword_score", "quality_score", "expected_score", "expected_decision"),
    [
        (3, 3, 3, "ASK_FOLLOW_UP"),
        (2, 5, 4, "NEXT_QUESTION"),  # round(3.5) == 4, at the threshold
        (4, 4, 4, "NEXT_QUESTION"),
        (0, 6, 3, "ASK_FOLLOW_UP"),
    ],
)
async def test_follow_up_is_asked_only_when_final_score_is_below_threshold(
    keyword_score, quality_score, expected_score, expected_decision
):
    evaluator = _evaluator_returning(
        '{"intent": "ANSWERING", "response": "", '
        f'"keyword_match_score": {keyword_score}, "answer_quality_score": {quality_score}, '
        '"decision": "NEXT_QUESTION", "feedback": ""}'
    )
    ...
    assert result["is_sufficient"] is (expected_decision == "NEXT_QUESTION")
